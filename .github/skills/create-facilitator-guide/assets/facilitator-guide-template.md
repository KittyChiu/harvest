# Workshop title

## Facilitator guide

**Purpose:**  
**Source of truth:**  
**Audience:**  
**Duration:**  
**Format:**  
**Group size:**  
**Prerequisites:**  
**Materials and tools:**  
**Participant output:**  

## Key takeaways

1. First approved takeaway
2. Second approved takeaway
3. Third approved takeaway

## Learning outcomes

| Participants will be able to | Evidence |
|---|---|
| Observable capability | Artifact, behaviour, or assessment |

## Takeaway journey

| Approved takeaway | Relatable experience | Teaching with or without demonstration | Practise | Debrief & call to action |
|---|---|---|---|---|
| Takeaway 1 | | | | |
| Takeaway 2 | | | | |
| Takeaway 3 | | | | |

## Agenda

| Time | Activity | Participant action | Outcome |
|---:|---|---|---|
| 0–10 min | Opening | | |

## Facilitator preparation

- Review the source of truth.
- Prepare the demonstration and fallback.
- Confirm tools, permissions, timing, and room setup.
- Prepare participant materials.

## Opening

### Purpose

### Talk track

### Audience prompt

### Transition

## Teach the core idea

### Source concept or model

### Explanation

### Example

### Boundary or common mistake

## Demonstration

**Purpose:**  
**Duration:**  
**Setup:**  
**Steps:**  
**What to notice:**  
**Fallback:**  

## Participant activity

**Purpose:**  
**Duration:**  
**Setup and materials:**  

### Facilitator instructions

### Participant instructions

### Expected output

### Debrief questions

### Watch-outs

### Fallback

## Debrief synthesis

Connect participant observations back to the guiding principles and constraints. Use a source model or framework only when one exists.

## Success criteria

- Observable result
- Reusable participant artifact
- Owner or next action

## Adaptations

### Shorter session

### Remote delivery

### Different audience or maturity

## Follow-up and transfer

State what participants should reuse, who owns it, and how learning continues after the session.
